...
$(document).ready(function() {
    function Banner() {
        ...
        var mouse = { x: 290, y: 250 };
        var mouse2 = { x: 275, y: 340 };
        var mouse1 = { x: 275, y: 430 };
        var mouse3 = { x: 410, y: 410 };
        var mouse4 = { x: 275, y: 500 };
        var mouse5 = { x: 290, y: 550 };
        var mouse6 = { x: 435, y: 235 };
        var mouse7 = { x: 380, y: 500 };
        var mouse8 = { x: 400, y: 160 };
        ...
        var start = function() {
            ...
            bgContext.font = "360px impact";
            bgContext.fillText(keyword, 125, 380);
        ...
        var getCoords = function() {
            ...
            for (height = 0; height < bgCanvas.height; height += denseness) {
                for (width = 0; width < bgCanvas.width; width += denseness) {
                    pixel = imageData.data[(width + height * bgCanvas.width) * 2 - 1];
                    if (pixel == 255) {
        ...
        var update = function() {
            var i, dx, dy, dx2, dy2, sqrDist, sqrDist2, scale;
            itercount++;
            fr = 0.979;
            // fr = 0.94;
            ...
            for (i = 0; i < parts.length; i++) {
                ...
                if (itercount == itertot) {
                    parts[i].v = { x: Math.random() * 7 - 7, y: Math.random() * 5 - 5 };
                    parts[i].r = false;
                ...
                dx1 = parts[i].x - mouse1.x;
                dy1 = parts[i].y - mouse1.y;
                sqrDist1 = Math.sqrt(dx1 * dx1 + dy1 * dy1);

                dx2 = parts[i].x - mouse2.x;
                dy2 = parts[i].y - mouse2.y;
                sqrDist2 = Math.sqrt(dx2 * dx2 + dy2 * dy2);

                dx3 = parts[i].x - mouse3.x;
                dy3 = parts[i].y - mouse3.y;
                sqrDist3 = Math.sqrt(dx3 * dx3 + dy3 * dy3);

                dx4 = parts[i].x - mouse4.x;
                dy4 = parts[i].y - mouse4.y;
                sqrDist4 = Math.sqrt(dx4 * dx4 + dy4 * dy4);

                dx5 = parts[i].x - mouse5.x;
                dy5 = parts[i].y - mouse5.y;
                sqrDist5 = Math.sqrt(dx5 * dx5 + dy5 * dy5);

                dx6 = parts[i].x - mouse6.x;
                dy6 = parts[i].y - mouse6.y;
                sqrDist6 = Math.sqrt(dx6 * dx6 + dy6 * dy6);

                dx7 = parts[i].x - mouse7.x;
                dy7 = parts[i].y - mouse7.y;
                sqrDist7 = Math.sqrt(dx7 * dx7 + dy7 * dy7);

                dx8 = parts[i].x - mouse8.x;
                dy8 = parts[i].y - mouse8.y;
                sqrDist8 = Math.sqrt(dx8 * dx8 + dy8 * dy8);
                ...
                context.beginPath();
                context.rect(parts[i].x2, parts[i].y2, 16, 16);
                context.closePath();
                context.fill();
            }
        };
...
$(document).ready(function() {
  // click question
  var toggle = 1;
  $(".hide_view_name").click(function() {
    $(".more_question").slideToggle("slow");
    if (toggle == 1) {
      $(".hide_view_name").text("Esconder");
      toggle--;
    } else if (toggle == 0) {
      $(".hide_view_name").text("Outras questões ");
      toggle++;
    }
  });
...
$(".section__button").on("click", "a", function(event) {
  event.preventDefault();
...
var block7Slider = function () {
    $(this).parents('.block7-top-left').toggleClass('active');
};
$(document).on('click', '.block7-composition-btn', block7Slider);


$('.sec-6__reviews-slider').on('beforeChange', function(event, slick, currentSlide, nextSlide){
    console.log(nextSlide);
    $('.block7-top-left').removeClass('active');
});

$(document).ready(function() {
  $(".cd-popup-trigger").on("click", function(s) {
    s.preventDefault(), $(".ever-popup").addClass("show");
  }),
    $(".ever-popup").on("click", function(s) {
      ($(s.target).is(".cd-popup-close") || $(s.target).is(".ever-popup")) &&
        (s.preventDefault(), $(this).removeClass("show"));
    }),
    $(document).keyup(function(s) {
      "27" == s.which && $(".ever-popup").removeClass("show");
    });
});

// click question
var toggle = 1;
$(".hide_view_name2").click(function() {
  $(".more-research").slideToggle("slow");
  if (toggle == 1) {
    $(".hide_view_name2").text("Esconder");
    toggle--;
  } else if (toggle == 0) {
    $(".hide_view_name2").text("Outros estudos");
    toggle++;
  }
});
